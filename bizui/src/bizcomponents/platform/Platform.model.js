...
export default {
  ...
  effects: {
    ...
    *addSkuInventory({ payload }, { call, put }) {
      const userContext = null
      const {PlatformService} = GlobalComponents;

      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.addSkuInventory, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/platform/${id}/list/\SkuInventoryList/Sku Inventory+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateSkuInventory({ payload }, { call, put }) {
      const userContext = null
      const {PlatformService} = GlobalComponents;      
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.updateSkuInventory, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/platform/${id}/list/\SkuInventoryList/Sku Inventory列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    *gotoNextSkuInventoryUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeSkuInventoryList({ payload }, { call, put }) {
     const userContext = null
      const {PlatformService} = GlobalComponents; 
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.removeSkuInventoryList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
