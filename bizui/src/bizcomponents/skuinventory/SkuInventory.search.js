...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{skuInventoryList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'product')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'product')}/> <PlatformAssociateForm 
...
class SkuInventorySearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {SkuInventoryTable} = GlobalComponents;
    const {SkuInventorySearchForm} = GlobalComponents;
    const {SkuInventoryModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <SkuInventorySearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,SkuInventoryModalTable,"skuInventoryIds")}
...
export default Form.create()(SkuInventorySearch)
