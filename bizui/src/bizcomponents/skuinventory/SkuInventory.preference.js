...
import styles from './SkuInventory.preference.less'
...
const internalImageListOf = (skuInventory) =>{
...
const internalSettingListOf = (skuInventory) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SkuInventoryService[candidateServiceName] 
...
const executeTrans = (skuInventory,targetComponent) =>{
...
const buildTransferModal = (skuInventory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(skuInventory,targetComponent)}
...
const internalRenderExtraHeader = (skuInventory) =>{
	return null
}
const internalRenderExtraFooter = (skuInventory) =>{
	return null
}
...
<Description term="Product">{skuInventory.product==null?"未分配":skuInventory.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Product","product","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",skuInventory.product?skuInventory.product.id:"")} 
...
const renderPermissionSetting = skuInventory => {
  const {SkuInventoryBase} = GlobalComponents
  return <PermissionSetting targetObject={skuInventory}  targetObjectMeta={SkuInventoryBase}/>


}


class SkuInventoryPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Sku Inventory",cardsFor: "skuInventory",cardsSource: this.props.skuInventory,
  		subItems: [
...
}))(Form.create()(SkuInventoryPreference))
